...
class AnomalyDetector:
    ...
    def load_and_prepare_data(self):
        ...
        df_train = df[df['role'] == 'normal'].select_dtypes(include=[
                                                            np.number])
    ...
    def train_and_predict(self):
        ...
        self.model.fit(self.df_train, self.df_train, epochs=50,
    ...
    def calculate_reconstruction_error(self, df_train_pred, df_test_pred):
        ...
        reconstruction_error_train = np.mean(
            np.power(self.df_train - df_train_pred, 2), axis=1)
        reconstruction_error_test = np.mean(
            np.power(self.df_test - df_test_pred, 2), axis=1)
        threshold = np.mean(reconstruction_error_train) + \
            3 * np.std(reconstruction_error_train)
        self.df_train['anomaly'] = (
            reconstruction_error_train > threshold).astype(int)
        self.df_test['anomaly'] = (
            reconstruction_error_test > threshold).astype(int)
        print(
            f"Número de anomalias na base de treinamento: {self.df_train['anomaly'].sum()}")
        print(
            f"Número de anomalias na base de teste: {self.df_test['anomaly'].sum()}")

    def report_anomalies(self):
        # Gera um relatório de anomalias com base em desvios padrão do conjunto de treinamento
        report = pd.DataFrame(columns=[
                              'offset_seconds', 'Registro'] + list(self.df_test.columns[:-1]) + ['TOTAL_Anom'])
        anom_stats = pd.DataFrame(
            columns=['Registro', 'Variable', 'Mean', 'Found Value', 'Deviation'])

        for index, row in self.df_test.iterrows():
            anomaly_row = [index, index]
            total_anom = 0
            # Excluindo coluna 'anomaly'
            for column in self.df_test.columns[:-1]:
                if row[column] > self.df_train[column].mean() + 3 * self.df_train[column].std():
                    ...
                    deviation = (found_value - mean_value) / \
                        self.df_train[column].std()
                    anom_stats.loc[len(anom_stats)] = [
                        index, column, mean_value, found_value, deviation]
                else:
    ...
    def run(self):
        ...
        report.to_excel('DADOS_SIMUL/FINAL_A2.xlsx',
                        index=False, sheet_name='RELATÓRIO')
...
if __name__ == "__main__":
    ...
    detector = AnomalyDetector(
        'BackEnd/NOVO_PETRO_BACK-main/DADOS/Adendo A.2_Conjunto de Dados_DataSet.csv')
    detector.run()
